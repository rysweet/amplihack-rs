...
pub fn adaptive_retrieve(
    ...
    if intent.intent.is_aggregation() {
        ...
        return RetrievalOutcome { exhaustive: false, used_simple_path: false, facts };
    }
    ...
    if use_simple {
        ...
        return RetrievalOutcome { used_simple_path: true, exhaustive, facts };
    }
    ...
    if facts.is_empty() {
        info!(question = &question[..question.len().min(50)],
            "Entity retrieval empty; falling back to simple retrieval");
        let facts = retriever.search(question, config.max_retrieval_limit);
        return RetrievalOutcome { used_simple_path: true, exhaustive: false, facts };
    }
    RetrievalOutcome { used_simple_path: false, exhaustive: false, facts }
}
...
pub fn supplement_keyword_expanded(
    ...
    if existing.len() >= config.keyword_expansion_sparse_threshold { return; }
...
pub fn dedup_and_filter(facts: Vec<MemoryFact>) -> Vec<MemoryFact> {
    ...
    facts.into_iter()
        .filter(|f| {
            if is_qa_echo(f) { return false; }
            if f.id.is_empty() { return true; }
...
fn is_small_kb(retriever: &dyn MemoryRetriever, question: &str, config: &SynthesisConfig) -> bool {
    retriever.search(question, config.simple_retrieval_threshold + 1).len()
        <= config.simple_retrieval_threshold
}

fn is_qa_echo(fact: &MemoryFact) -> bool {
    fact.context.starts_with("Question:")
        && fact.metadata.get("tags").and_then(|v| v.as_array())
...
fn extract_supplement_keywords(question: &str) -> Vec<String> {
    let stop: HashSet<&str> = [
        ...
        "many", "much", "does", "did", "was", "were", "are", "do", "which", "who",
        "where", "when", "why", "that", "this", "with", "from", "by", "it", "be",
        "as", "on", "at", "not", "but", "if", "so", "about", "than", "its", "has",
        "have", "had", "can",
    ].into_iter().collect();
...
fn extract_entity_mentions(question: &str) -> Vec<String> {
    question.split_whitespace()
        .filter(|w| {
            ...
            !t.is_empty()
                && (t.chars().next().is_some_and(|c| c.is_uppercase()) || t.contains('-'))
        })
...
mod tests {
    ...
    fn qa_fact(id: &str) -> MemoryFact {
        ...
        MemoryFact { id: id.into(), context: "Question: X?".into(),
            outcome: "Answer: Y".into(), confidence: 0.7, metadata: meta }
    }

    #[test]
    fn simple_intent_uses_simple_path() {
        let r = FakeRetriever(vec![mf("1", "ctx", "out")]);
        let i = DetectedIntent { intent: super::super::types::IntentType::SimpleRecall, ..Default::default() };
    ...
    fn aggregation_intent() {
        let r = FakeRetriever(vec![mf("1", "ctx", "out")]);
        let i = DetectedIntent { intent: super::super::types::IntentType::MetaMemory, ..Default::default() };
    ...
    fn dedup_and_filter_works() {
        let facts = vec![mf("1", "a", "b"), mf("1", "a", "b"), qa_fact("q1"), mf("2", "c", "d")];
    ...
    fn supplement_adds_new() {
        ...
        let cfg = SynthesisConfig { keyword_expansion_sparse_threshold: 50, ..Default::default() };
    ...
    fn supplement_skipped_above_threshold() {
        let mut existing: Vec<MemoryFact> = (0..50).map(|i| mf(&format!("f{i}"), "c", "o")).collect();
        let len = existing.len();
        let cfg = SynthesisConfig { keyword_expansion_sparse_threshold: 30, ..Default::default() };
