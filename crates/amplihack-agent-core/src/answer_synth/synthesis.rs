...
pub async fn synthesize(
    ...
    let max_facts = if intent.intent.is_complex() {
        config.max_facts_complex
    } else {
        config.max_facts_simple
    };
    ...
    info!(
        question = &question[..question.len().min(80)],
        total_facts = facts.len().min(max_facts),
        "SYNTH_DIAG starting synthesis"
    );
    let messages = [
        LlmMessage::system(
            "You are a knowledgeable assistant that synthesizes accurate \
            answers from retrieved facts. Answer precisely based on the provided context.",
        ),
        LlmMessage::user(prompt),
    ];
    match llm
        .completion(&messages, &config.model, config.synthesis_temperature)
        .await
    {
...
fn format_facts(facts: &[MemoryFact], max_facts: usize, intent: &DetectedIntent) -> String {
    let temporal = intent.needs_temporal
        || matches!(
            intent.intent,
            IntentType::TemporalComparison | IntentType::IncrementalUpdate
        );
    let header = if temporal {
        "Relevant facts (ordered chronologically where possible):\n"
    } else {
        "Relevant facts:\n"
    };
...
fn build_extra_instructions(
    ...
    if intent.intent.is_complex() {
        if let Some(cat) = category_instruction(intent.intent) {
            extra.push_str(cat);
    ...
    {
        let label = if tc.operation == "change_count" {
            "Change count"
        } else {
            "Value"
        };
...
fn append_cue_instructions(
    ...
    if intent.intent == IntentType::ContradictionResolution
        || level == QuestionLevel::L5
        || has(&[
            "disagree",
            "conflicting",
            "contradiction",
...
mod tests {
    ...
    fn extra_for_incident() {
        let e = build_extra_instructions(
            "CVE-2024-001 breach?",
            QuestionLevel::L1,
            &DetectedIntent::default(),
        );
    ...
    fn extra_for_contradiction() {
        let i = DetectedIntent {
            intent: IntentType::ContradictionResolution,
            ..Default::default()
        };
    ...
    fn extra_math_precomputed() {
        let i = DetectedIntent {
            intent: IntentType::MathematicalComputation,
            needs_math: true,
            computed_math: Some("42".into()),
            ..Default::default()
        };
