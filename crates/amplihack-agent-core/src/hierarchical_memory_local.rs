...
impl HierarchicalMemoryLocal {
    pub fn new(agent_name: impl Into<String>) -> Self {
        ...
        Self { agent_name: name, nodes: Vec::new(), edges: Vec::new(),
               episodes: Vec::new(), next_id: 1 }
    }
    ...
    pub fn store_knowledge(&mut self, p: StoreKnowledgeParams<'_>) -> String {
        ...
        if let Some(tm) = p.temporal_metadata { metadata.extend(tm.clone()); }
        let node = KnowledgeNode {
            node_id: node_id.clone(), category: p.category,
            content: p.content.trim().to_string(), concept: p.concept.trim().to_string(),
            confidence: p.confidence, source_id: p.source_id.to_string(),
            created_at: chrono::Utc::now().to_rfc3339(), tags: p.tags.to_vec(), metadata,
        };
        let new_edges: Vec<KnowledgeEdge> = self.nodes.iter().filter_map(|existing| {
            let sim = compute_word_similarity(&node.content, &existing.content);
            if sim >= 0.3 {
                Some(KnowledgeEdge {
                    source_id: node.node_id.clone(), target_id: existing.node_id.clone(),
                    relationship: "SIMILAR_TO".into(), weight: sim,
    ...
    pub fn store_episode(&mut self, content: &str, source_label: &str) -> String {
        ...
        self.episodes.push(KnowledgeNode {
            node_id: eid.clone(), category: MemoryCategory::Episodic,
            content: content.to_string(), concept: source_label.to_string(),
            confidence: 1.0, source_id: String::new(),
            created_at: chrono::Utc::now().to_rfc3339(),
            tags: Vec::new(), metadata: HashMap::new(),
        });
        eid
    }
    ...
    pub fn retrieve_subgraph(&self, query: &str, max_nodes: usize) -> KnowledgeSubgraph {
        if query.trim().is_empty() { return KnowledgeSubgraph::new(query); }
        let lower = query.to_lowercase();
        let keywords: Vec<&str> = lower.split_whitespace().filter(|w| w.len() > 2).collect();
        let mut scored: Vec<(f64, &KnowledgeNode)> = self.nodes.iter().filter_map(|n| {
            let text = format!("{} {}", n.concept, n.content).to_lowercase();
            let hits = keywords.iter().filter(|k| text.contains(**k)).count();
            if hits > 0 { Some((hits as f64 * n.confidence, n)) } else { None }
        }).collect();
        scored.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
        let nodes: Vec<KnowledgeNode> = scored.into_iter().take(max_nodes).map(|(_, n)| n.clone()).collect();
        let ids: std::collections::HashSet<&str> = nodes.iter().map(|n| n.node_id.as_str()).collect();
        let edges: Vec<KnowledgeEdge> = self.edges.iter()
            .filter(|e| ids.contains(e.source_id.as_str()) || ids.contains(e.target_id.as_str()))
            .cloned().collect();
        KnowledgeSubgraph { nodes, edges, query: query.to_string() }
    }
    ...
    pub fn retrieve_by_entity(&self, entity: &str, limit: usize) -> Vec<KnowledgeNode> {
        ...
        self.nodes.iter().filter(|n| {
            n.concept.to_lowercase().contains(&lower) || n.content.to_lowercase().contains(&lower)
        }).take(limit).cloned().collect()
    }

    pub fn execute_aggregation(&self, query_type: &str, entity_filter: &str) -> HashMap<String, serde_json::Value> {
        let mut r = HashMap::new();
        match query_type {
            "count_entities" => {
                let count = if entity_filter.is_empty() { self.nodes.len() } else {
                    let lf = entity_filter.to_lowercase();
                    self.nodes.iter().filter(|n| n.concept.to_lowercase().contains(&lf)).count()
                };
            ...
            _ => { r.insert("error".into(), serde_json::json!(format!("unsupported: {query_type}"))); }
        }
        r
    }
    ...
    pub fn get_statistics(&self) -> HashMap<String, serde_json::Value> {
        HashMap::from([
            ...
            ("total_episodes".into(), serde_json::json!(self.episodes.len())),
        ])
    }
    ...
    pub fn import_from_json(&mut self, data: &serde_json::Value, merge: bool) -> HashMap<String, serde_json::Value> {
        if !merge { self.nodes.clear(); self.edges.clear(); self.episodes.clear(); }
        let (mut in_n, mut in_e) = (0usize, 0usize);
        if let Some(arr) = data.get("nodes").and_then(|v| v.as_array()) {
            for v in arr { if let Ok(n) = serde_json::from_value::<KnowledgeNode>(v.clone()) { self.nodes.push(n); in_n += 1; } }
        }
        if let Some(arr) = data.get("edges").and_then(|v| v.as_array()) {
            for v in arr { if let Ok(e) = serde_json::from_value::<KnowledgeEdge>(v.clone()) { self.edges.push(e); in_e += 1; } }
        }
        HashMap::from([
...
mod tests {
    ...
    fn store(mem: &mut HierarchicalMemoryLocal, content: &str, concept: &str, conf: f64) -> String {
        mem.store_knowledge(StoreKnowledgeParams {
            content, concept, confidence: conf, category: MemoryCategory::Semantic,
    ...
    fn store_retrieve_and_edges() {
        ...
        store(&mut mem, "photosynthesis converts light energy", "Biology", 0.9);
        store(&mut mem, "photosynthesis produces oxygen from light", "Biology", 0.8);
    ...
    fn aggregation_episode_stats() {
        ...
        assert_eq!(mem.execute_aggregation("count_entities", "")["count"], serde_json::json!(2));
        assert_eq!(mem.execute_aggregation("list_concepts", "")["concepts"].as_array().unwrap().len(), 2);
    ...
    fn empty_name() { HierarchicalMemoryLocal::new(""); }
}
