...
fn ensure_node_for_copilot() -> Result<Option<PathBuf>> {
    ...
    amplihack_utils::litellm_proxy::scrub_inference_environment(&mut extract_cmd);
    extract_cmd
...
fn run_npm_install(npm: &Path, prefix: &Path, package: &str) -> Result<()> {
    ...
    amplihack_utils::litellm_proxy::scrub_inference_environment(&mut npm_cmd);
    npm_cmd
        .arg("install")
        .arg("-g")
        .arg("--prefix")
...
fn install_amplifier() -> Result<()> {
    ...
    amplihack_utils::litellm_proxy::scrub_inference_environment(&mut uv_cmd);
    uv_cmd
        .arg("tool")
        .arg("install")
