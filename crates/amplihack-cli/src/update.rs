    ...
    #[cfg(unix)]
    {
        ...
        if ready < 0 {
            return Err(io::Error::last_os_error())
                .context("failed waiting for update prompt input");
        }
        if ready == 0 {
            println!();
            return Ok(None);
...
mod tests {
    ...
    #[test]
    fn startup_update_prompt_accepts_yes_variants() {
        assert!(wants_startup_update(Some("y")));
        assert!(wants_startup_update(Some("YES")));
        assert!(wants_startup_update(Some(" yes ")));
    }

    #[test]
    fn startup_update_prompt_defaults_to_no() {
        assert!(!wants_startup_update(None));
        assert!(!wants_startup_update(Some("")));
        assert!(!wants_startup_update(Some("n")));
        assert!(!wants_startup_update(Some("later")));
    }
