...
use chrono::Utc;
use uuid::Uuid;
use crate::models::{HiveFact, FACT_ID_HEX_LENGTH};
...
impl DistributedCognitiveMemory {
    pub fn new(hive: DistributedHiveGraph, agent_name: impl Into<String>, quality_threshold: f64) -> Self {
        Self { local_facts: Vec::new(), hive, agent_name: agent_name.into(), quality_threshold }
    }

    pub fn agent_name(&self) -> &str { &self.agent_name }
    pub fn local_fact_count(&self) -> usize { self.local_facts.len() }
    pub fn hive(&self) -> &DistributedHiveGraph { &self.hive }
    pub fn hive_mut(&mut self) -> &mut DistributedHiveGraph { &mut self.hive }

    /// Store a fact locally and auto-promote to the hive if quality passes.
    pub fn store_fact(&mut self, concept: &str, content: &str, confidence: f64, tags: Vec<String>) -> String {
        let fact_id = format!("{:0>width$}", &Uuid::new_v4().to_string().replace('-', "")[..FACT_ID_HEX_LENGTH],
    ...
    pub fn search_facts(&self, query: &str, limit: usize, _min_confidence: f64) -> Vec<serde_json::Value> {
    ...
    pub fn search_by_concept(&self, keywords: &[&str], limit: usize) -> Vec<serde_json::Value> {
        let query = keywords.iter().take(super::QUERY_KEYWORD_LIMIT).copied()
    ...
    pub fn local_search_facts(&self, query: &str, limit: usize) -> Vec<serde_json::Value> {
        let mut scored: Vec<(f64, &serde_json::Value)> = self.local_facts.iter()
    ...
    fn query_hive(&self, query: &str, limit: usize) -> Vec<serde_json::Value> {
        self.hive.query_facts(query, limit).into_iter().map(|sf| shard_fact_to_dict(&sf)).collect()
    }

    fn get_all_hive_facts(&self, limit: usize) -> Vec<serde_json::Value> {
        self.hive.query_facts("", limit).into_iter().map(|sf| shard_fact_to_dict(&sf)).collect()
    }
    ...
    fn auto_promote(&mut self, concept: &str, content: &str, confidence: f64, tags: &[String]) {
        ...
        let fact = HiveFact {
            fact_id: format!("{:0>width$}", &Uuid::new_v4().to_string().replace('-', "")[..FACT_ID_HEX_LENGTH],
                width = FACT_ID_HEX_LENGTH),
            concept: concept.to_string(), content: content.to_string(), confidence,
            source_id: self.agent_name.clone(), tags: tags.to_vec(),
...
fn shard_fact_to_dict(fact: &crate::dht::ShardFact) -> serde_json::Value {
    ...
    for tag in &fact.tags {
        if let Some(date) = tag.strip_prefix("date:") { meta.insert("date".into(), serde_json::json!(date)); }
        else if let Some(time) = tag.strip_prefix("time:") { meta.insert("time".into(), serde_json::json!(time)); }
    }
    serde_json::json!({
...
mod tests {
    ...
    fn shard_fact_to_dict_restores_metadata() {
        let fact = crate::dht::ShardFact {
            fact_id: "f1".into(), content: "content".into(), concept: "concept".into(),
            confidence: 0.9, source_agent: "agent".into(),
            tags: vec!["date:2024-01-01".into(), "time:12:00".into()],
            created_at: 0.0, metadata: HashMap::new(), ring_position: 0,
        };
