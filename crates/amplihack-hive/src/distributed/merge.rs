...
pub fn merge_ranked_shard_results(
    results_by_agent: &[(String, Vec<ShardFact>)], limit: usize, pos_decrement: f64,
) -> Vec<ShardFact> {
    ...
    for (_, facts) in sorted {
        for (rank, fact) in facts.iter().enumerate() {
            ...
            best.entry(key).and_modify(|(s, f)| {
                if score > *s || (score == *s && tiebreak(fact) < tiebreak(f)) {
                    *s = score; *f = fact.clone();
                }
...
pub fn merge_aggregation_results(
    query_type: &str, results: &[HashMap<String, serde_json::Value>],
) -> HashMap<String, serde_json::Value> {
    let mut merged = HashMap::new();
    match query_type {
        "count_total" => {
            let total: u64 = results.iter()
                .filter_map(|r| r.get("count").and_then(|v| v.as_u64())).sum();
        ...
        "list_entities" | "list_concepts" | "list_superseded" => {
            let mut items: Vec<String> = results.iter()
                ...
                .flatten().filter_map(|v| v.as_str().map(|s| s.to_string()))
        ...
        _ => {
            if let Some(first) = results.iter().find(|r| !r.is_empty()) { return first.clone(); }
        }
    }
    merged
}
...
pub fn relevance_score(content: &str, query: &str) -> f64 {
    ...
    let qbigrams: Vec<(String, String)> = qt.windows(2).map(|w| (w[0].clone(), w[1].clone())).collect();
    let cwl: Vec<String> = cl.split_whitespace().map(|w| w.to_string()).collect();
    let cbigrams: HashSet<(&str, &str)> = cwl.windows(2).map(|w| (w[0].as_str(), w[1].as_str())).collect();
    let bhits = qbigrams.iter().filter(|(a, b)| cbigrams.contains(&(a.as_str(), b.as_str()))).count();
...
pub fn extract_content(result: &serde_json::Value) -> String {
    for key in &["outcome", "content", "fact"] {
        if let Some(s) = result.get(key).and_then(|v| v.as_str()) { return s.to_string(); }
    }
    String::new()
}
...
pub fn merge_fact_lists(
    local: &[serde_json::Value], hive: &[serde_json::Value], limit: usize, query: &str,
    ...
    for item in local.iter().chain(hive.iter()) {
        ...
        if seen.insert(h) {
            let score = if query.is_empty() { 0.5 } else { relevance_score(&c, query) };
...
mod tests {
    ...
    fn sfact(id: &str, content: &str, agent: &str) -> ShardFact {
        ShardFact { fact_id: id.into(), content: content.into(), source_agent: agent.into(),
    ...
    fn rrf_merge_respects_limit() {
        let r = vec![("a1".into(), vec![
            sfact("f1", "r1", "a1"), sfact("f2", "r2", "a1"), sfact("f3", "r3", "a1")])];
    ...
    fn aggregation_count_total() {
        ...
        assert_eq!(merge_aggregation_results("count_total", &results)["count"], 8);
    }
    ...
    #[test]
    fn relevance_score_full_match() { assert!(relevance_score("Rust memory safety", "Rust memory safety") > 0.9); }

    #[test]
    fn relevance_score_no_match() { assert!(relevance_score("Python is great", "Rust memory") < 0.01); }

    #[test]
    fn merge_fact_lists_dedup() {
        let local = vec![serde_json::json!({"content": "fact A"})];
        let hive = vec![serde_json::json!({"content": "fact A"}), serde_json::json!({"content": "fact B"})];
