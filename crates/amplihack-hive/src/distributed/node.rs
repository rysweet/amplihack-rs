...
use crate::models::{make_event, BusEvent, HiveFact, PEER_CONFIDENCE_DISCOUNT};
...
impl AgentNode {
    pub fn new(agent_id: impl Into<String>, domain: impl Into<String>) -> Self {
        ...
        Self {
            ...
            agent_id: id, domain: domain.into(),
            bus: None, coordinator: None,
            incorporated_ids: VecDeque::new(), incorporated_set: HashSet::new(),
        }
    }

    pub fn agent_id(&self) -> &str { &self.agent_id }
    pub fn domain(&self) -> &str { &self.domain }
    pub fn is_connected(&self) -> bool { self.bus.is_some() }
    pub fn fact_count(&self) -> usize { self.graph.fact_count() }
    pub fn graph(&self) -> &HiveGraph { &self.graph }

    /// Store a fact locally, publish FACT_LEARNED event, report to coordinator.
    pub fn learn(
        &mut self, concept: &str, content: &str, confidence: f64, tags: Option<Vec<String>>,
    ) -> crate::error::Result<String> {
        let fact_id = self.graph.store_fact(
            concept, content, confidence, &self.agent_id, tags.unwrap_or_default(),
        )?;
        if let Some(bus) = &self.bus {
            ...
            if let Ok(mut bus) = bus.lock() { let _ = bus.publish(make_event("FACT_LEARNED", &self.agent_id, payload)); }
        }
        if let Some(coord) = &self.coordinator
            && let Ok(mut coord) = coord.lock() { coord.report_fact(&self.agent_id, concept); }
        Ok(fact_id)
    }

    pub fn query(&self, query: &str, limit: usize) -> Vec<HiveFact> {
        self.graph.query_facts(query, 0.0, limit).unwrap_or_default()
    }
    ...
    pub fn incorporate_peer_fact(&mut self, event: &BusEvent) -> bool {
        if self.incorporated_set.contains(&event.event_id) { return false; }
        if event.source_id == self.agent_id { return false; }
        let concept = event.payload.get("concept").and_then(|v| v.as_str()).unwrap_or("");
        let content = event.payload.get("content").and_then(|v| v.as_str()).unwrap_or("");
        let confidence = event.payload.get("confidence").and_then(|v| v.as_f64()).unwrap_or(0.5);
        if content.is_empty() { return false; }
        let discounted = confidence * PEER_CONFIDENCE_DISCOUNT;
        let source = event.payload.get("source_agent").and_then(|v| v.as_str()).unwrap_or("");
        let tags = vec![format!("peer_source:{source}")];
        let ok = self.graph.store_fact(concept, content, discounted, &self.agent_id, tags).is_ok();
        if ok { self.track_incorporated(&event.event_id); }
        ok
    }

    pub fn join_hive(&mut self, bus: Arc<Mutex<LocalEventBus>>, coordinator: Arc<Mutex<HiveCoordinator>>) {
        if let Ok(mut b) = bus.lock() {
            let _ = b.subscribe(&self.agent_id, Some(&["FACT_LEARNED", "FACT_PROMOTED"]));
        }
        if let Ok(mut c) = coordinator.lock() { c.register_agent(&self.agent_id, &self.domain); }
        self.bus = Some(bus);
    ...
    pub fn leave_hive(&mut self) {
        if let Some(bus) = self.bus.take()
            && let Ok(mut b) = bus.lock() { let _ = b.unsubscribe(&self.agent_id); }
        if let Some(coord) = self.coordinator.take()
            && let Ok(mut c) = coord.lock() { c.unregister_agent(&self.agent_id); }
    }

    pub fn process_pending_events(&mut self) -> usize {
        let events = if let Some(bus) = &self.bus {
            bus.lock().ok().and_then(|mut b| b.poll(&self.agent_id).ok()).unwrap_or_default()
        } else { return 0; };
        let mut count = 0;
        for event in &events {
            if self.incorporate_peer_fact(event) { count += 1; }
        }
        count
    }
...
mod tests {
    ...
    fn learn_stores_locally() {
        ...
        let fid = node.learn("rust", "Rust is memory-safe", 0.9, None).unwrap();
        assert!(!fid.is_empty());
    ...
    fn query_returns_matching_facts() {
        ...
        node.learn("rust", "Rust ownership model", 0.9, None).unwrap();
        node.learn("python", "Python is interpreted", 0.8, None).unwrap();
    ...
    fn incorporate_peer_fact_applies_discount() {
        ...
        let event = make_event("FACT_LEARNED", "a2",
    ...
    fn incorporate_skips_own_events() {
        let mut node = AgentNode::new("a1", "d");
        let event = make_event("FACT_LEARNED", "a1",
