...
use crate::models::{make_event, BusEvent};
...
pub trait ShardTransport {
    ...
    fn retrieve_by_entity_shard(&self, agent_id: &str, entity: &str, limit: usize) -> Vec<ShardFact>;
    fn store_on_shard(&mut self, agent_id: &str, fact: ShardFact);
    fn execute_aggregation_shard(&self, agent_id: &str, query_type: &str, entity_filter: &str)
        -> HashMap<String, serde_json::Value>;
}

/// In-process transport that accesses the DHT router directly.
pub struct LocalShardTransport { router: Arc<Mutex<DHTRouter>> }

impl LocalShardTransport {
    pub fn new(router: Arc<Mutex<DHTRouter>>) -> Self { Self { router } }
}

impl ShardTransport for LocalShardTransport {
    fn query_shard(&self, agent_id: &str, query: &str, limit: usize) -> Vec<ShardFact> {
        let r = match self.router.lock() { Ok(r) => r, Err(_) => return Vec::new() };
        r.get_shard(agent_id).map(|s| s.search(query, limit).into_iter().cloned().collect())
            .unwrap_or_default()
    }
    fn retrieve_by_entity_shard(&self, agent_id: &str, entity: &str, limit: usize) -> Vec<ShardFact> {
    ...
    fn execute_aggregation_shard(&self, agent_id: &str, query_type: &str, _entity_filter: &str)
        -> HashMap<String, serde_json::Value> {
        let r = match self.router.lock() { Ok(r) => r, Err(_) => return HashMap::new() };
        let shard = match r.get_shard(agent_id) { Some(s) => s, None => return HashMap::new() };
        let mut result = HashMap::new();
        match query_type {
            "count_total" => { result.insert("count".into(), serde_json::json!(shard.fact_count())); }
            "list_concepts" => {
                let concepts: Vec<String> = shard.all_facts().iter()
                    .map(|f| f.concept.clone()).collect::<std::collections::HashSet<_>>()
                    .into_iter().collect();
...
impl EventBusShardTransport {
    pub fn new(bus: Arc<Mutex<LocalEventBus>>, agent_id: impl Into<String>) -> Self {
        Self { bus, agent_id: agent_id.into(), local_router: None }
    }
    pub fn with_local_router(mut self, router: Arc<Mutex<DHTRouter>>) -> Self {
        self.local_router = Some(router); self
    }
    ...
    fn local_query(&self, query: &str, limit: usize) -> Vec<ShardFact> {
        if let Some(router) = &self.local_router
            && let Ok(r) = router.lock() {
                return r.get_shard(&self.agent_id)
    ...
    fn publish_shard_event(&self, topic: &str, payload: serde_json::Value) {
        ...
        if let Ok(mut bus) = self.bus.lock() { let _ = bus.publish(event); }
    }
}

impl ShardTransport for EventBusShardTransport {
    fn query_shard(&self, agent_id: &str, query: &str, limit: usize) -> Vec<ShardFact> {
        if self.is_local(agent_id) { return self.local_query(query, limit); }
        self.publish_shard_event("SHARD_QUERY", serde_json::json!({
    ...
    fn store_on_shard(&mut self, agent_id: &str, fact: ShardFact) {
        if self.is_local(agent_id)
            ...
            && let Ok(mut r) = router.lock() { r.store_fact(fact); return; }
        self.publish_shard_event("SHARD_STORE", serde_json::json!({
    ...
    fn execute_aggregation_shard(&self, agent_id: &str, query_type: &str, entity_filter: &str)
        ...
        self.publish_shard_event("SHARD_AGGREGATION", serde_json::json!({
...
pub fn handle_shard_store(event: &BusEvent, router: &Mutex<DHTRouter>) {
    if let Some(fv) = event.payload.get("fact")
        ...
        && let Ok(mut r) = router.lock() { r.store_fact(fact); }
}
...
mod tests {
    ...
    fn local_transport_query() {
        ...
        { let mut r = router.lock().unwrap(); r.store_fact(ShardFact::new("f1", "Rust is memory-safe")); }
    ...
    fn local_transport_aggregation() {
        let router = make_router();
        { let mut r = router.lock().unwrap(); r.store_fact(ShardFact::new("f1", "some fact content")); }
        let t = LocalShardTransport::new(Arc::clone(&router));
    ...
    fn event_bus_transport_remote_publishes() {
        ...
        { let mut b = bus.lock().unwrap(); b.subscribe("listener", Some(&["SHARD_QUERY"])).unwrap(); }
    ...
    fn handle_shard_store_event() {
        ...
        let event = make_event("SHARD_STORE", "a2",
            serde_json::json!({"fact": serde_json::to_value(&fact).unwrap()}));
        handle_shard_store(&event, &router);
        let r = router.lock().unwrap();
        let total: usize = r.all_agents().iter()
            .filter_map(|a| r.get_shard(a)).map(|s| s.fact_count()).sum();
        assert!(total >= 1);
    }
}
