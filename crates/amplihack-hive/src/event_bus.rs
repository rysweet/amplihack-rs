...
impl EventBus for LocalEventBus {
    fn publish(&mut self, event: BusEvent) -> Result<()> {
        ...
        for agent_id in agent_ids {
            // Skip self-delivery
            if agent_id == event.source_id { continue; }
...
mod tests {
    ...
    #[test]
    fn self_delivery_skipped() {
        let mut bus = LocalEventBus::new();
        bus.subscribe("agent-1", None).unwrap();
        bus.publish(ev("t", "agent-1")).unwrap();
        assert!(bus.pending_events("agent-1").unwrap().is_empty());
    }
