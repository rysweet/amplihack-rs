...
pub fn make_learn_content_event(source: &str, content: &str) -> BusEvent {
...
pub fn make_feed_complete_event(feed_id: &str, items: u32) -> BusEvent {
...
pub fn make_agent_ready_event(agent_id: &str) -> BusEvent {
...
pub fn make_query_event(question: &str) -> (String, BusEvent) {
...
mod tests {
    ...
    fn make_learn_content_event_has_correct_topic() {
        let event = make_learn_content_event("test-source", "hello world");
    ...
    fn make_feed_complete_event_has_correct_topic() {
        let event = make_feed_complete_event("feed-1", 10);
    ...
    fn make_agent_ready_event_has_correct_topic() {
        let event = make_agent_ready_event("agent-42");
        assert_eq!(event.topic, HIVE_AGENT_READY);
    }

    #[test]
    fn make_query_event_returns_query_id() {
        let (query_id, event) = make_query_event("What is Rust?");
    ...
    fn make_query_response_event_has_confidence() {
        let event = make_query_response_event("q1", "A language", 0.85);
    ...
    fn events_have_unique_ids() {
        let e1 = make_learn_content_event("s", "c");
        let e2 = make_learn_content_event("s", "c");
