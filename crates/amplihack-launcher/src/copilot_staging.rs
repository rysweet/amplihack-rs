...
fn generate_hook_wrapper(hook_name: &str, py_path: &Path, rust_binary: Option<&Path>) -> String {
    ...
    // PR #3916 port: resilient repo root detection — fall back to $(pwd), never empty.
    script.push_str(
        "REPO_ROOT=\"$(git rev-parse --show-toplevel 2>/dev/null)\" || REPO_ROOT=\"$(pwd)\"\n",
    );
    script.push_str("export REPO_ROOT\n");
...
mod tests {
    ...
    fn hook_wrapper_rust() {
        ...
        assert!(w.contains("REPO_ROOT"));
        assert!(w.contains(r#"|| REPO_ROOT="$(pwd)""#));
    }
    #[test]
