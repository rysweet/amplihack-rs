...
impl ClaudeLauncher {
    pub fn new(config: LauncherConfig) -> Self {
        Self { config, path_cache: HashMap::new() }
    }

    /// Full pre-launch preparation. Returns `true` if launch can proceed.
    pub fn prepare_launch(&mut self) -> Result<bool> {
        if is_noninteractive() && !self.check_prerequisites_noninteractive()? { return Ok(false); }
        if !self.handle_repo_checkout()? { return Ok(false); }
    ...
    pub fn build_claude_command(&self) -> Result<Command> {
        ...
        if let Some(ref pf) = self.config.append_system_prompt {
            if pf.exists() { cmd.args(["--append-system-prompt", &pf.to_string_lossy()]); }
            else { warn!(path = %pf.display(), "System prompt file not found"); }
        }
        ...
        for arg in &self.config.claude_args { cmd.arg(arg); }
        cmd.env("AMPLIHACK_MANAGED", "1");
        if self.config.verbose { cmd.env("AMPLIHACK_VERBOSE", "1"); }
        debug!(cli = %cli_path.display(), "Built Claude command");
        Ok(cmd)
    }

    pub fn launch(&mut self) -> Result<i32> {
        if !self.prepare_launch()? { return Ok(1); }
    ...
    pub fn launch_interactive(&mut self) -> Result<i32> {
        ...
        Ok(cmd.status().context("failed to launch")?.code().unwrap_or(1))
    }

    pub fn invalidate_path_cache(&mut self) { self.path_cache.clear(); }
    pub fn config(&self) -> &LauncherConfig { &self.config }

    fn check_prerequisites_noninteractive(&self) -> Result<bool> {
        if get_claude_cli_path().is_err() { warn!("Claude CLI not found in non-interactive mode"); return Ok(false); }
        Ok(true)
    }

    fn handle_repo_checkout(&self) -> Result<bool> {
        let Some(ref repo) = self.config.checkout_repo else { return Ok(true); };
        info!(repo = %repo, "Checking out repository...");
        let status = Command::new("git").args(["clone", "--depth", "1", repo]).status().context("failed to run git clone")?;
        if !status.success() { warn!(repo = %repo, "Repository checkout failed"); return Ok(false); }
        Ok(true)
    }

    fn find_target_directory(&self) -> Option<PathBuf> {
        if let Some(ref dir) = self.config.target_directory && dir.exists() { return Some(dir.clone()); }
        let mut current = std::env::current_dir().ok()?;
        loop {
            if current.join(".claude").exists() { return Some(current); }
    ...
    fn fix_hook_paths_in_settings(&self, target_dir: &Path) -> Result<()> {
        ...
        let content = std::fs::read_to_string(&sp).with_context(|| format!("read {}", sp.display()))?;
        if !content.contains(PROJECT_DIR_PLACEHOLDER) { return Ok(()); }
...
pub fn is_sandboxed() -> bool {
    ["CI", "GITHUB_ACTIONS", "DOCKER_CONTAINER", "KUBERNETES_SERVICE_HOST"]
        .iter().any(|v| std::env::var(v).is_ok())
        || std::env::var("HOME").is_err() || std::env::var("PATH").is_err()
}

fn get_claude_cli_path() -> Result<PathBuf> {
    if let Ok(p) = std::env::var("CLAUDE_CLI_PATH") { let pb = PathBuf::from(p); if pb.exists() { return Ok(pb); } }
    which_binary("claude").ok_or_else(|| anyhow::anyhow!("Claude CLI not found"))
}

fn paths_are_same(a: &Path, b: &Path) -> bool {
    match (a.canonicalize(), b.canonicalize()) { (Ok(ca), Ok(cb)) => ca == cb, _ => a == b }
}
...
#[cfg(unix)]
fn is_tty() -> bool { unsafe { libc::isatty(0) != 0 } }
#[cfg(not(unix))]
fn is_tty() -> bool { false }

pub fn launch_claude(args: &[String]) -> Result<i32> {
    ClaudeLauncher::new(LauncherConfig { claude_args: args.to_vec(), ..Default::default() }).launch()
}
...
mod tests {
    ...
    #[test]
    fn default_config() { let c = LauncherConfig::default(); assert!(!c.verbose); assert!(c.claude_args.is_empty()); }
    #[test]
    fn config_serializes() {
        let c = LauncherConfig { verbose: true, claude_args: vec!["--model".into(), "opus".into()], ..Default::default() };
        let j = serde_json::to_value(&c).unwrap();
        assert_eq!(j["verbose"], true); assert_eq!(j["claude_args"][0], "--model");
    }
    #[test]
    fn launcher_stores_config() { assert!(ClaudeLauncher::new(LauncherConfig { verbose: true, ..Default::default() }).config().verbose); }
    #[test]
    ...
    #[test]
    fn paths_same_identical() { assert!(paths_are_same(Path::new("/usr"), Path::new("/usr"))); }
    #[test]
    fn paths_same_different() { assert!(!paths_are_same(Path::new("/usr"), Path::new("/var"))); }
    #[test]
    fn ensure_runtime_dirs() {
        let dir = tempfile::tempdir().unwrap();
        ClaudeLauncher::new(LauncherConfig::default()).ensure_runtime_directories(dir.path()).unwrap();
        for n in RUNTIME_DIRS { assert!(dir.path().join(".claude/runtime").join(n).exists(), "Missing: {n}"); }
    }
    #[test]
    fn fix_hook_paths_replaces() {
        ...
        std::fs::write(&sp, r#"{"hooks": {"path": "$CLAUDE_PROJECT_DIR/hooks/pre.sh"}}"#).unwrap();
        ClaudeLauncher::new(LauncherConfig::default()).fix_hook_paths_in_settings(dir.path()).unwrap();
        let c = std::fs::read_to_string(&sp).unwrap();
        assert!(!c.contains("$CLAUDE_PROJECT_DIR")); assert!(c.contains(&dir.path().to_string_lossy().to_string()));
    }
    #[test]
    ...
    fn find_target_from_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join(".claude")).unwrap();
        let l = ClaudeLauncher::new(LauncherConfig { target_directory: Some(dir.path().into()), ..Default::default() });
