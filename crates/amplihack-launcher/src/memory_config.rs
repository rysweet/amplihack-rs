...
pub fn parse_node_options(options_str: &str) -> Vec<(String, String)> {
    options_str.split_whitespace().map(|token| {
...
pub fn merge_node_options(existing: &str, new_limit_mb: u64) -> String {
    ...
    for token in existing.split_whitespace() {
        if token.starts_with("--max-old-space-size") || token.starts_with("--max_old_space_size") {
            if !replaced { parts.push(format!("--max-old-space-size={new_limit_mb}")); replaced = true; }
        } else { parts.push(token.to_string()); }
    }
    if !replaced { parts.push(format!("--max-old-space-size={new_limit_mb}")); }
    parts.join(" ")
}
...
pub fn save_user_preference(consent: bool, limit_mb: u64) -> Result<()> {
    save_preference_to(&get_config_path().ok_or_else(|| anyhow::anyhow!("no config path"))?, consent, limit_mb)
}
...
pub fn get_memory_config(existing_node_options: Option<&str>) -> MemoryConfig {
    let Some(ram_gb) = detect_system_ram_gb() else {
        ...
        return MemoryConfig {
            system_ram_gb: None, recommended_limit_mb: MIN_MEMORY_MB,
            current_limit_mb: None, node_options: existing_node_options.unwrap_or_default().into(),
            warning: None, user_consent: None, returning_user: false,
    ...
    MemoryConfig {
        system_ram_gb: Some(ram_gb), recommended_limit_mb: recommended,
        current_limit_mb: current_limit, node_options, warning,
        user_consent: saved.map(|p| p.consent), returning_user, error: None,
    }
}
...
pub fn display_memory_config(config: &MemoryConfig) -> String {
    ...
    lines.push(format!("ℹ Recommended limit: {} MB", config.recommended_limit_mb));
    if let Some(c) = config.current_limit_mb { lines.push(format!("ℹ Current limit: {c} MB")); }
    if let Some(ref w) = config.warning { lines.push(format!("⚠ {w}")); }
    lines.join("\n")
}
...
fn load_preference_from(path: &Path) -> Result<MemoryPreference> {
    ...
    let section = value.get("memory_config").ok_or_else(|| anyhow::anyhow!("no memory_config"))?;
...
fn save_preference_to(path: &Path, consent: bool, limit_mb: u64) -> Result<()> {
    ...
    std::fs::write(path, serde_json::to_string_pretty(&config)?).with_context(|| format!("write {}", path.display()))?;
    info!(consent, limit_mb, "Saved memory preference");
    Ok(())
}

fn round_to_power_of_2(gb_float: f64) -> u64 {
    if gb_float <= 0.0 { return 0; }
    let nearest_pow2 = 2_u64.pow(gb_float.log2().round() as u32);
    let ratio = gb_float / nearest_pow2 as f64;
    if (0.75..=1.25).contains(&ratio) { nearest_pow2 } else { gb_float.round() as u64 }
}
...
fn detect_ram_windows() -> Option<u64> {
    let out = std::process::Command::new("wmic").args(["ComputerSystem", "get", "TotalPhysicalMemory"]).output().ok()?;
    if !out.status.success() { return None; }
...
mod tests {
    ...
    #[test]
    fn calculate_limit_16gb() { assert_eq!(calculate_recommended_limit(16), MIN_MEMORY_MB); }
    #[test]
    fn calculate_limit_64gb() { assert_eq!(calculate_recommended_limit(64), 16384); }
    #[test]
    fn calculate_limit_256gb() { assert_eq!(calculate_recommended_limit(256), MAX_MEMORY_MB); }
    #[test]
    fn warn_below_min() { assert!(should_warn_about_limit(4096)); assert!(!should_warn_about_limit(8192)); }
    #[test]
    ...
    #[test]
    fn parse_empty() { assert!(parse_node_options("").is_empty()); }
    #[test]
    fn merge_replaces() { assert_eq!(merge_node_options("--max-old-space-size=4096 --no-warnings", 8192), "--max-old-space-size=8192 --no-warnings"); }
    #[test]
    fn merge_adds() { assert_eq!(merge_node_options("--no-warnings", 8192), "--no-warnings --max-old-space-size=8192"); }
    #[test]
    fn merge_empty() { assert_eq!(merge_node_options("", 16384), "--max-old-space-size=16384"); }
    #[test]
    fn extract_found() { assert_eq!(extract_max_old_space("--max-old-space-size=4096"), Some(4096)); }
    #[test]
    fn extract_missing() { assert_eq!(extract_max_old_space("--no-warnings"), None); }
    #[test]
    fn round_pow2_exact() { assert_eq!(round_to_power_of_2(16.0), 16); assert_eq!(round_to_power_of_2(64.0), 64); }
    #[test]
    fn round_pow2_close() { assert_eq!(round_to_power_of_2(15.8), 16); assert_eq!(round_to_power_of_2(31.5), 32); }
    #[test]
    fn config_serializes() {
        let c = MemoryConfig { system_ram_gb: Some(64), recommended_limit_mb: 16384, current_limit_mb: Some(4096),
            node_options: "--max-old-space-size=16384".into(), warning: None, user_consent: Some(true), returning_user: true, error: None };
    ...
    fn preference_round_trips() {
        let p = MemoryPreference { consent: true, limit_mb: 16384, node_options: None };
        let parsed: MemoryPreference = serde_json::from_str(&serde_json::to_string(&p).unwrap()).unwrap();
        assert!(parsed.consent); assert_eq!(parsed.limit_mb, 16384);
    }
    #[test]
    fn save_and_load() {
        ...
        assert!(loaded.consent); assert_eq!(loaded.limit_mb, 16384);
    }
    #[test]
    fn save_merges_existing() {
        ...
        let c: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        assert_eq!(c["other_key"], "value"); assert_eq!(c["memory_config"]["consent"], false);
    }
    #[test]
    fn get_config_returns_result() {
        let c = get_memory_config(Some("--no-warnings"));
        assert!(!c.node_options.is_empty()); assert!(c.recommended_limit_mb >= MIN_MEMORY_MB);
    }
    #[test]
    fn display_format() {
        let c = MemoryConfig { system_ram_gb: Some(32), recommended_limit_mb: 8192, current_limit_mb: Some(4096),
            node_options: String::new(), warning: Some("low memory".into()), user_consent: None, returning_user: false, error: None };
        let out = display_memory_config(&c);
        assert!(out.contains("32 GB")); assert!(out.contains("8192 MB")); assert!(out.contains("low memory"));
    }
}
