...
// ── Performance contracts (from Python) ──────────────────────────
...
impl BenchmarkEvaluator {
    ...
    ///
    /// Stores and retrieves `num_operations` entries, measuring elapsed time.
    pub fn evaluate(
...
/// Evaluates retrieval quality by running test queries against a backend
/// and comparing results to ground-truth relevant IDs.
///
...
impl RetrievalQualityEvaluator {
    ...
    ///
    /// Each `QueryTestCase` contains a query and the IDs of memories that
    /// are considered relevant. The evaluator retrieves results from the
    /// backend and computes precision, recall, relevance, and NDCG.
    pub fn evaluate(
...
///
/// Uses the bit_length optimization from the Python implementation:
/// `bit_length()` approximates `floor(log2(n)) + 1`, giving a similar
/// discount curve to `1/log2(i+2)`.
fn calculate_ndcg(retrieved: &[&str], relevant: &HashSet<&str>) -> f64 {
    ...
    for (i, id) in retrieved.iter().enumerate() {
        if relevant.contains(id) {
            // bit_length of (i+2): number of bits needed to represent i+2
...
///
/// Stores diverse memories across all five cognitive types and returns
/// test queries whose relevant IDs point to the stored entries.
...
mod tests {
    ...
    fn ndcg_perfect_score() {
        ...
        let score = calculate_ndcg(&retrieved, &relevant);
    ...
    fn ndcg_empty_relevant() {
        ...
        let retrieved = vec!["a"];
        assert_eq!(calculate_ndcg(&retrieved, &relevant), 0.0);
    }
    ...
    fn ndcg_partial_match() {
        let relevant: HashSet<&str> = ["a", "b"].into_iter().collect();
        let retrieved = vec!["c", "a"]; // only "a" at position 1
        let score = calculate_ndcg(&retrieved, &relevant);
