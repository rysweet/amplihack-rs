...
/// and error recovery.
///
/// The Python version tests concurrent safety via asyncio. In sync Rust the
/// borrow checker prevents data races, so we verify rapid sequential
/// operations instead.
...
impl BackendReliabilityEvaluator {
    ...
    fn test_data_integrity(backend: &mut dyn MemoryBackend) -> f64 {
        ...
        for (content, mt) in &test_cases {
            ...
            if let Ok(results) = backend.retrieve(&query)
                && results
                    .iter()
                    .any(|r| r.id == stored_id && r.content == *content)
            {
                successes += 1;
            }
        }
    ...
    /// Rapid sequential store operations.
    ...
    fn test_error_recovery(backend: &mut dyn MemoryBackend) -> f64 {
        ...
        match backend.retrieve(&MemoryQuery::new("")) {
