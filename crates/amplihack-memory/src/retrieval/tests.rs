...
use std::collections::HashMap;
    ...
    impl MemorySearch for MockMemory {
        ...
        fn retrieve_by_entity(&self, entity_name: &str, limit: usize) -> Vec<Fact> {
            ...
            self.facts
                .iter()
                .filter(|f| {
                    f.context.to_lowercase().contains(&name)
                        || f.outcome.to_lowercase().contains(&name)
                })
                .take(limit)
                .cloned()
                .collect()
    ...
    fn infra_retrieval_extracts_subnet_names() {
        ...
        let result = infrastructure_relation_retrieval(
            &mem,
            "what subnet is used?",
            &existing,
            false,
    ...
    fn source_filter_finds_matching() {
        ...
        let result = filter_facts_by_source_reference(
            "mentioned in the athlete achievements article?",
            &facts,
        );
