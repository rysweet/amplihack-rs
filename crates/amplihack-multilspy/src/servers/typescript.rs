...
impl LanguageServer for TypeScriptServer {
    async fn start(&mut self) -> Result<()> {
            ...
            .on_notification(
                ...
                Box::new(|params| {
                    if let Some(msg) = params.get("message").and_then(|v| v.as_str()) {
                        info!("typescript-language-server: {msg}");
                    }
                }),
            )
            .await;
        ...
        info!("typescript-language-server ready");
