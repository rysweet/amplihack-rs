...
pub async fn execute_remote_workflow(
    ...
    let mut packager = ContextPackager::new(repo_path, 500, skip_secret_scan);
    ...
    let vm = orchestrator.provision_or_reuse(vm_options).await?;
    info!(vm = %vm.name, "VM ready");

    // Step 4: Transfer context
    info!("transferring context");
    let executor = Executor::new(vm.clone(), timeout_minutes, vm_options.tunnel_port);
    ...
    info!(command, max_turns, "executing remote command");
    let result = executor.execute_remote(command, prompt, max_turns).await?;
    ...
    if !vm_options.keep_vm && (result.exit_code == 0 || result.timed_out) {
