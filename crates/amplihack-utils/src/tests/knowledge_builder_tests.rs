...
impl ArtifactGenerator for StubArtifactGen {
    fn generate_all(&self, _graph: &KnowledgeGraph) -> Result<Vec<PathBuf>, KnowledgeBuilderError> {
...
impl QuestionGenerator for FailingQuestionGen {
    fn generate_all_questions(&self, _topic: &str) -> Result<Vec<Question>, KnowledgeBuilderError> {
        Err(KnowledgeBuilderError::PipelineStep(
            "question gen failed".into(),
        ))
    }
}
...
fn make_builder(
    ...
    let config = KnowledgeBuilderConfig::new(
        topic,
        Some("test-agent".into()),
        Some(PathBuf::from("/fake/output")),
    );
...
fn config_custom_output() {
    let cfg = KnowledgeBuilderConfig::new(
        "AI Safety",
        Some("my-agent".into()),
