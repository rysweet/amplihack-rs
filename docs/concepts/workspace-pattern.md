...
git clone https://github.com/rysweet/amplihack-rs.git my-workspace
cd my-workspace
...
**Agent specialization per project.** amplihack's powerful agent system (including specialized agents like `/dev` and `/amplihack:modular-build`) becomes available to all your projects. Each project can have its own AGENTS.md that leverages these specialized agents while maintaining project-specific context.
...
**Command integration.** amplihack's extensive command library (accessible through slash commands) becomes available across all your projects. Commands like `/dev` for deep analysis or `/amplihack:fix` for code improvements work seamlessly within project boundaries.
...
# Fork rysweet/amplihack-rs on GitHub, then:
git clone https://github.com/yourusername/amplihack-rs.git my-workspace
...
- **Agent support**: Use `/dev` for architectural analysis
- **Code improvements**: Use `/amplihack:fix` for refactoring tasks
````
...
```
/dev - Deep analysis with extended thinking time
...
```
Working on @my-blog/. Use /dev to analyze the best approach
...
- **Complex refactoring**: Use `/dev` to analyze impact across
...
**Leverage amplihack agents.** Don't forget to use specialized commands like `/dev` for deep analysis or `/amplihack:modular-build` for modular design work. These agents understand both amplihack's philosophy and your project's constraints.
...
Yes! All amplihack commands (like `/dev`, `/amplihack:modular-build`, etc.) work seamlessly within your project. They have access to both your project's AGENTS.md and amplihack's workspace-level philosophy documents.

**Q: How do I install and run amplihack?**

amplihack can be run with the `amplihack` CLI from your workspace directory. Refer to the [First-Time Install](../howto/first-install.md) guide for installation options.
