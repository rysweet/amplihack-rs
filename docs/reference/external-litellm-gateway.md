...
## Child environment ordering

The launcher applies its canonical `EnvBuilder` environment before gateway
policy. Gateway cleanup then removes configuration sources, direct-provider
credentials, and conflicting routing selectors before the target adapter
projects the validated endpoint and virtual key.

Gateway cleanup and projection are the final child-environment mutations. No
ordinary environment builder runs afterward, so it cannot restore a removed
credential or override the validated route.
