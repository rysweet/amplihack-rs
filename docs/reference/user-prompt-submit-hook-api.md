...
- **English only.** Each transcript turn of a memory is checked on its own. A turn is a paragraph that starts with one of those role labels. Any other paragraph continues the current turn and keeps its words. That includes one that opens with a lower-case word and a colon (`sqlite: …`). A role-label line inside a closed fenced block is ambiguous: it may be a pasted log, or a real turn boundary with a stray fence line before it. The memory is scored under both readings, and it must be relevant under both, which fails closed. Only turns whose prose reads as English contribute topic words: at least 10% of their prose words must be common English function words (`the`, `and`, `with`, `because`, …) or English contractions.
  ...
  - Code is ignored when judging the language: fenced blocks (```` ``` ```` or `~~~`, closed by a line of at least as many of the same character, also inside block quotes and list items) and closed backtick spans of any width, as CommonMark defines them, and tokens with digits or symbols in them (paths, flags, `snake_case`, `DEFAULT_TIMEOUT`, `sha256`). The one deliberate difference from CommonMark is that an unclosed fence opens nothing, since session-stop cuts memories at 500 characters. Words written in capitals (`CI`, `THE`) are still words. A code span that holds a sentence (4 or more words) is judged on its own for scoring. If the span isn't English, like a German error pasted in a fence after `fix this:`, only its code-looking tokens are scored, not its words. A prompt or turn with no prose outside code (`/fix` plus a pasted error in a fence) is judged on the words inside its code spans. If those words aren't English, the prompt gets no memories and the turn contributes nothing, and that includes its code-looking tokens. Surrounding punctuation and quotes in any script (`¿`, `“`, `»`) are trimmed first, so they don't make a word look like code.
  ...
  The prompt is held to the same check. A prompt of 4 or more prose or topic words that doesn't read as English gets no memories, so a German prompt's `die`, `bin` or `mit` can't match those words in an English memory. This also fails closed:
  - A long English prompt with no function words (`/fix flaky sqlite test timeout on linux ci`) gets no memories.
  - A prompt with 3 or fewer prose words *and* 3 or fewer topic words (`/analyze user login`) is too short to judge, so it isn't checked. A prompt whose scored words are mostly code (paths, identifiers) is still judged.
